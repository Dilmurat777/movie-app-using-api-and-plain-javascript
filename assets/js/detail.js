...
import { api_key, imageBaseURL, fetchDataFromSever } from './api.js';
...
const getGenres = function(genresList) {
	const newGenreList = [];

	for (const { name } of genresList) newGenreList.push(name);
...
fetchDataFromSever(`https://api.themoviedb.org/3/movie/${movieId}?api_key=c08a5b544a250b00781a187cb551af8f&append_to_response=casts,videos,images,releases`, function(movie){
	...
	fetchDataFromSever(`https://api.themoviedb.org/3/movie/${movieId}/recommendations?api_key=${api_key}&page=1`, addSuggestedMovies);
